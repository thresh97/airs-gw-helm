## AIRS Gateway Configuration Variables
...
| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| `imageCredentials[].name` | string | `"airsgatewayregistrycredentials"` | Name of the image pull secret |
...
```bash
kubectl create secret generic airs-gw-credentials \
...
| `environment.data.AZURE_AUTH_MODE` | string | `""` | Azure authentication mode (`workload`, `managed`, `entra`) |
...
| `environment.data.GCP_AUTH_MODE` | string | `""` | GCP authentication mode (`workload`). Required for both GCS log storage and Vertex AI workload identity. See [Vertex AI Workload Identity](./VertexAI.md) |
| `environment.data.GCP_WIF_AUDIENCE` | string | `""` | Workload Identity Federation audience (used with `GCP_AUTH_MODE=workload` outside GKE; when unset the GKE metadata server is used). See [Vertex AI Workload Identity](./VertexAI.md) |
